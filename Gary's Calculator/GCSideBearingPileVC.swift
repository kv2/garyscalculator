...
class GCSideBearingPileVC: UIViewController,UITextFieldDelegate,UIPickerViewDataSource,UIPickerViewDelegate {
    ...
    @IBOutlet weak var textFieldPileLength: UITextField!
    ...
    @IBOutlet weak var textFieldPileCircumferecnce: UITextField!
    ...
    @IBOutlet weak var texteFieldNumberOfPilesNeeded: UITextField!
    ...
    var numberOfPilesRequired: Float = 0.0
    ...
    func donePickerPileSection(){
        ...
        let translationNumber = 4 * numberTranslateTextfield
...
//        }
   ...
   //MARK: - Delegates and data sources
    ...
    func checkIfLoadValuesAreSet() -> Bool{
        ...
        let factorOfSafety = Float(textFieldfactorOfSafety.text!)
        let pileLength = Float(textFieldPileLength.text!)
    ...
    func calculateDeducedValues() -> Bool{
        ...
        var pileCircumference: Float = 0.0
        
        
        if(pileSection == "Round"){
            
            pileCircumference = Float.pi * 2 * pileDimension/12.0
            
        } else {
            
            pileCircumference = pileDimension/12.0 * 4.0
            
        }
        
        textFieldPileCircumferecnce.text = String.localizedStringWithFormat("%.2f", pileCircumference)
        
        let totalLoadOnBuilding = bldgSquareFootage * (liveloadDeduced + bldgDeadLoad)
        textFieldTotalLoadOfBuilding.text = String.localizedStringWithFormat("%.0f", totalLoadOnBuilding)

        
        numberOfPilesRequired = totalLoadOnBuilding * factorOfSafety / (pileCircumference * pileLength * soilLatealBearingCapacity)
    ...
    @IBAction func calculateValues(){
        ...
        texteFieldNumberOfPilesNeeded.text = String.localizedStringWithFormat("%d", Int(ceil(numberOfPilesRequired)))
